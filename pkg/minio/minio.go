...
func DownloadFile(bucketName string, objectName string, fileDownloadPath string) error {
	err := MinioClient.FGetObject(context.Background(), bucketName, objectName, fileDownloadPath, minio.GetObjectOptions{})
	if err != nil {
		return err
	}
	return nil
...
func PresignedGetObjectUrl(ctx context.Context, bucketNameViper string, objectName string, expires time.Duration) (string, error) {
	viper := viper.Init("minio")
	bucketName := viper.GetString(bucketNameViper)
